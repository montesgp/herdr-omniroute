...
.DESCRIPTION
  ...
  preference order:
    ...
    1. sqlite3 (default)
       ...
       launching `netstat` creates zero console windows, which matters twice here
       because the widget runs it over a transparent always-on-top window.
  ...
  Dot-source it, do not run it:
    ...
    . (Join-Path $PSScriptRoot "Read-SqliteQuery.ps1")
#>
...
# The PATH is scanned by hand instead of with Get-Command: the panel read runs on the
# UI thread while the user waits, and Get-Command's first call in a process costs
# ~60 ms warming up the command discovery machinery, which is real latency for a
# lookup of one file.
...
.PARAMETER BusyTimeoutMs
  ...
  fast so the caller can show "data unavailable" instead of freezing the bar.
...
function Invoke-SqliteQuery {
  ...
  # costs ~70 ms of PowerShell provider initialisation, and this read happens while
  # the user is looking at the bar waiting for the panel.
